...
public class OpenTelemetryConfig {
    ...
    public OpenTelemetry openTelemetry() {
        ...
        ZipkinSpanExporter zipkinExporter = ZipkinSpanExporter.builder().setEndpoint("http://localhost:9414/api/v2/spans").build();
        // Define the resource with a service name for trace attribution
        Resource serviceResource = // Service name for trace identification
        Resource.builder().put(ResourceAttributes.SERVICE_NAME, "ProfilingService").build();
        // Configure the SDK tracer provider with the batch span processor and service resource
        SdkTracerProvider sdkTracerProvider = // Associate traces with the defined service resource
        // Batch span processor for efficient export
        SdkTracerProvider.builder().addSpanProcessor(BatchSpanProcessor.builder(zipkinExporter).build()).setResource(serviceResource).build();
        // Build and return the OpenTelemetry SDK instance
        return // Attach the configured tracer provider
     ...
     *
     * @param openTelemetry
     * 		the OpenTelemetry instance to retrieve the tracer from.
